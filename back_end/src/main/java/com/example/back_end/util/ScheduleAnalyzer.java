...
public class ScheduleAnalyzer {
    ...
    //             timeSlotMap.computeIfAbsent(timeSlot, k -> new ArrayList<>()).add(assignment);
    //         }
    //     }
    ...
    //             classroomMap.computeIfAbsent(classroomId, k -> new ArrayList<>()).add(assignment);
    //         }
        
    //         // 检查同一时间槽内是否有多个任务使用同一个教室
    //         for (Map.Entry<String, List<Assignment>> classroomEntry : classroomMap.entrySet()) {
    //             if (classroomEntry.getValue().size() > 1) {
    //                 timeConflicts++;
    //                 System.out.println(" 时间冲突: 时间槽 " + entry.getKey() + " 教室 " + classroomEntry.getKey() + " 被以下任务占用:");
    ...
    //             System.out.println("  未排入课程: " + assignment.getTask().getCourseName() + " (指定教室: " + classroomId + ")");
    ...
    //                         System.out.println("    - 时间槽 " + entry.getKey() + ": " + occupyingAssignment.getTask().getCourseName());
    //                     }
    //                 }
    //             }
    //         }
    ...
    public Map<String, Double> calculateBuildingUtilization() {
        ...
        for (Map.Entry<String, Integer> entry : buildingUsage.entrySet()) {
            ...
            double utilizationRate = classroomCount > 0 ? (double) usedSlots / (classroomCount * totalSlotsPerWeek) : 0.0;
        ...
        for (Map.Entry<String, Double> entry : buildingUtilization.entrySet()) {
            System.out.println("  教学楼 " + entry.getKey() + ": 使用率 " + String.format("%.2f", entry.getValue() * 100) + "%");
        }
    ...
    //         teacherAssignments.computeIfAbsent(teacherId, k -> new ArrayList<>()).add(assignment);
    //         classAssignments.computeIfAbsent(classId, k -> new ArrayList<>()).add(assignment);
    //     }

    //     System.out.println("教师安排冲突:");
    //     for (Map.Entry<String, List<Assignment>> entry : teacherAssignments.entrySet()) {
