...
public class AssignmentController {
    ...
    public ResponseEntity<ApiResponseDTO<List<AssignmentDTO>>> getAssignmentsByClassroom(
    ...
    public ResponseEntity<ApiResponseDTO<List<AssignmentDTO>>> getConflicts() {
        try {
            List<Assignment> conflicts = assignmentService.getConflicts();
            List<AssignmentDTO> conflictDTOs = conflicts.stream()
                    .map(this::convertToDTO)
                    .collect(Collectors.toList());
            return ResponseEntity.ok(ApiResponseDTO.success("获取冲突排课结果成功", conflictDTOs));
        } catch (Exception e) {
    ...
    private AssignmentDTO convertToDTO(Assignment entity) {
        ...
        dto.setTeachingClassId(entity.getTeachingClassId());
